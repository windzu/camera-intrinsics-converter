...
Examples:
  ...
  convert-intrinsics sensing input.txt output.yaml \\
    ...
    parser.add_argument(
        'input',
        type=str,
        help='Input intrinsics file path'
    )
...
def convert_intrinsics(
    ...
    if verbose:
        ...
        print(f"  Original intrinsics: fx={data['fx']:.4f}, fy={data['fy']:.4f}, cx={data['cx']:.4f}, cy={data['cy']:.4f}")
    ...
    if data.get('has_rational_model'):
        ...
        is_significant, message = converter.check_rational_model_significance(k4, k5, k6)
        ...
        if verbose:
            print(f"  Fitting RMS error: {fitted_result['fitting_error_rms']:.6f} (normalized coordinates)")
